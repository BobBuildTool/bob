...
function run_single_tests () {
  ...
  if [[ ! $LOCAL ]]; then
    expect_success run_bob archive ${BOB_ARGS} scan --fail -v
    expect_success run_bob archive ${BOB_ARGS} clean --fail -v 'metaEnv.TYPE == "alpha"'
    rm -rf work $archiveDir/*
...
function run_multi_tests () {
  ...
  expect_success run_bob archive -a scan
  # clean up archive databses to allow proper rescanning
  rm .bob-archive*
  # scan single archive "local"
  expect_success run_bob archive -b "local" scan
  rm .bob-archive*
  # scan single archive "local2"
  expect_success run_bob archive -b "local2" scan
  rm .bob-archive*
