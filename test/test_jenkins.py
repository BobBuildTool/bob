...
class TestJenkinsPush(TestCase):
    def executeBobJenkinsCmd(self, arg, workingDir):
        doJenkins(arg.split(' '), workingDir)
        try:
            finalize()
...
packageScript: |
    TestPackageScript
        ...
        with TemporaryDirectory() as tmp:
            ...
            # do bob jenkins add
            self.executeBobJenkinsCmd("add myTestJenkins http://localhost:8080 -r test", tmp)
            # throw away server data (but there shouldn't by any...
            assert(len(self.jenkinsMock.getServerData()) == 0)

            self.executeBobJenkinsCmd("push myTestJenkins -q", tmp)
            ...
            self.executeBobJenkinsCmd("prune -q myTestJenkins", tmp)
            send = self.jenkinsMock.getServerData()
            assert(len(send) == 1)
            assert( '/job/test/doDelete'  ==  send[0][0])

            self.executeBobJenkinsCmd("rm myTestJenkins", tmp)
            assert(len(self.jenkinsMock.getServerData()) == 0)
...
buildScript: |
    true
        ...
        with TemporaryDirectory() as tmp:
            ...
            # do bob jenkins add
            self.executeBobJenkinsCmd("add myTestJenkins http://localhost:8080 -r root", tmp)
